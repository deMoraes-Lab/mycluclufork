...
plot_tsne <- function(plot_data, ititle, isubtitle) {
  mysam <- function(x) {
    if_else(
      sample(c(TRUE, FALSE), length(x), replace = TRUE, prob = c(0.58, 0.42)),
      x,
      NA
    )
  }
  ...
  ggplot(plot_data) +
    geom_jitter(aes(x = mysam(V1), y = mysam(V2), color = fct_lump(cluster, 8)), size = 5, shape = 1, alpha = 2 / 3) +
    ...
    scale_color_manual(values = c(paletteer::paletteer_d("ggthemes::few_Light"), "black")) +
    labs(
      title = ititle,
      subtitle = isubtitle,
...
x$noise <- rnorm(nrow(hits_char), mean = 0.0, sd = 1e-5)
...
if (RECALCULATE) {
  ...
  write_rds(perplexity_search, PSEARCH)
} else {
...
for (i in seq_along(perplexity_search)) {
  ...
  subtitle <- glue("{HYPER} HDB_clusters")
...
no_outliers_plots <- vector(mode = "list", length = length(PERPLEXITIES))
names(no_outliers_plots) <- perplexity_names
...
for (i in seq_along(perplexity_search)) {
  ...
  plot_data$cluster <- as_factor(HDB_wdl$cluster)
  plot_data$outlier <- HDB_wdl$cluster == 0
  plot_data <- filter(plot_data, !outlier, cluster != 0)

  title <- glue("HDB non-outliers {names(perplexity_search)[i]}")
  subtitle <- glue("{HYPER} non-outliers HDB_clusters")
  perplexity_plots[[i]] <- plot_tsne(plot_data, title, subtitle)
}
...
for (i in seq_along(perplexity_search)) {
  ...
  subtitle <- glue("{HYPER} PFAM curated clusters")
...
for (i in seq_along(perplexity_search)) {
  ...
  subtitle <- glue("{HYPER} Phylum clusters")
  phylum_plots[[i]] <- plot_tsne(plot_data, title, subtitle)
}

# 44444444444444444444444444444

curated_NO_plots <- vector(mode = "list", length = length(PERPLEXITIES))
names(curated_NO_plots) <- perplexity_names
...
for (i in seq_along(perplexity_search)) {
  ...
  title <- glue("PFAM non-outliers {names(curated_NO_plots)[i]}")
  subtitle <- glue("{HYPER} curated NO outliers")
  phylum_plots[[i]] <- plot_tsne(plot_data, title, subtitle)
}
...
for (i in seq_along(curated_NO_plots)) {
  ggsave(glue("curatedNO_{i}.pdf"), plot = curated_NO_plots[[i]], width = 8.5, height = 11, units = "in", dpi = 300)
}

for (i in seq_along(no_outliers_plots)) {
  ggsave(glue("NOhdb_{i}.pdf"), plot = no_outliers_plots[[i]], width = 8.5, height = 11, units = "in", dpi = 300)
}


# combine
system("pdfunite hdbclust_* phylum_* curated_* curatedNO_* NOhdb_* scaled.pdf", intern = TRUE)
